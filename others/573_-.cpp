...
int32_t main()
{
    double h, f, b, d, count=0;
    while (scanf("%llf %llf %llf %llf", &h, &f, &b, &d), h){
        double reached=0;
        int count=0;
        while (1){
            if (f* d/100 *count < f) reached += f - f* d/100 *count;
            // bug(reached, count);
            if (reached > h){
                print("success on day " << count+1);
                break;
            }
            reached -= b;
            if (reached < 0){
                print("failure on day " << count+1);
                break;
            }
            count++;
        }
