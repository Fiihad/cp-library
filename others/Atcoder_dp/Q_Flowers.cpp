...
int rec(int i, int j){
    ...
    if (j > height[i]) return rec(i+1, j);
    return max(rec(i+1, j), rec(i+1, height[i]) + beauty[i]);
}
...
#define right(node) (left(node) + 1)
    ...
    int query(int start, int ending, int l, int r, int node)
    {
        ...
        if (start > r || ending < l) { return LLONG_MIN; }
...
void solve(int cases)
{
    ...
    height.resize(n+1);
    beauty.resize(n+1);
    #define scanx(array) {loop(i,1,(array.size())) cin >> array[i]; }
    scanx(height); scanx(beauty);
    // {for (__typeof((height.size()+1)) i = (1) - ((1) > ((height.size()+1))); i != ((height.size()+1)) - ((1) > ((height.size()+1))); i += 1 - 2 * ((1) > ((height.size()+1))))
    //     bug(i), cin >> height[i]; }
    // {for (__typeof((beauty.size()+1)) i = (1) - ((1) > ((beauty.size()+1))); i != ((beauty.size()+1)) - ((1) > ((beauty.size()+1))); i += 1 - 2 * ((1) > ((beauty.size()+1)))) cin >> beauty[i]; }

    vi hashing(n+1);
    loop(i,1,n+1) hashing[height[i]] = i;
    ...
    loop(i,1,n+1){
        int index = hashing[i];
        dp[index] = beauty[index] + st.query(1, index-1);
        // bug(index, beauty[index], st.query(1, index-1));
