...
// SOLVE STARTS HERE
bool poss(int value, int q, vi need, vi have){
    loop(i,0,need.size()){
        if (have[i]/need[i]>=value) continue;
        if (need[i]*value<=have[i]+q) q -= need[i]*value-have[i];
        else return false;
    }
    return true;
}
...
void solve(int cases)
{
    ...
    vi need(n), have(n);
    scanx(need); scanx(have);

    int hi=*max_element(all(have))+q, lo=0;
    while(hi>=lo){
        int mid= (hi+lo)/2;

        if (poss(mid, q, need, have)) out=mid, lo=mid+1;
        else hi=mid-1;
    }
