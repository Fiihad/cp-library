...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        multiset<int> b;
        scanx(a);
        loop(i,n) cin >> in, b.insert(in);

        loop(i, n) cout << *b.lower_bound(a[i]) - a[i] << gap;
        cout << endl;
        vector<int>vtt;
        for(int i=n-1;i>=0;i--)
        {
            vtt.push_back(*b.rbegin()-a[i]);
        ...
        //     maxo.push_front((*max_element(all(b)) - a[i]));
        //     b.erase(*lower_bound(all(b), a[i]));
        // }
        // loop(i, n) cout << maxo[i] << gap;
        // cout << endl;
    }
    return 0;
}
