...
void solve(int cases)
{
    ...
    int firstzero = lower_bound(all(presum), 0) - presum.begin();
    int lastzero = lower_bound(all(presum), presum.back()) - presum.begin();

    int r=lastzero,l=firstzero;
    sum = last.back();
    ...
    while(1){
        ...
        if (i>=j || i<firstzero || j<firstzero) break;
    }
    pii dp = {l+1, r+1};

    while (q--){
        int start,finish,l,r;
        cin >> start >> finish;
        if (start<=dp.first && finish>=dp.second){
            print(dp.first << gap << dp.second);
            continue;
        }
        start--, finish--;

        r=min(finish, lastzero),l=max(start, firstzero);
        if (l>0) sum = presum[r]-presum[l-1] - (prexor[r]^prexor[l-1]);
        else sum= presum[r]-prexor[r];

        int i=min(finish, lastzero), j=min(finish, lastzero), cost;
        while(1){
            if(i>0) cost = (presum[j]-presum[i-1])-(prexor[j]^prexor[i-1]);
            else cost = presum[j]-prexor[j];
            if (cost==sum){
