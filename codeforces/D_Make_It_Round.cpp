...
void solve(int cases)
{
    int n, m, out=0, sum=1, flag=0, maxi=LLONG_MIN, mini=LLONG_MAX;
    int in;
    cin >> n >> m;

    int count2=0, count5=0;
    for (int i=flag=n; i%10==0 && i; i/=10) flag/=10;
    for (int i=flag; i%2==0 && i; i/=2) count2++;
    for (int i=flag; i%5==0 && i; i/=5) count5++;

    while(1){
        if (count5-->0 && m/2) sum*=2, m/=2;
        else if (count2-->0 && m/5) sum*=5, m/=5;
        else if (m/10) sum*=10, m/=10;
        else break;
    }
    m *= sum;
