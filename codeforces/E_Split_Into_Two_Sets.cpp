...
// SOLVE STARTS HERE
int dfs(vector<vi> &adj_list, vector<int> &color, int node)
{
    for (int nbr: adj_list[node]){
        if (color[nbr] == color[node]) return 0;
        if (color[nbr]) continue;
        color[nbr] = color[node]==1? 2:1;
        if (!dfs(adj_list, color, nbr)) return 0;
    }
    return 1;
}
...
void solve(int cases)
{
    int n, q, out=0, sum=0, self = 0, flag=1, maxi=LLONG_MIN, mini=LLONG_MAX;
    int in;
    cin >> n;

    vector<vi> adj_list(n+1);
    vi color(n+1, 0), freq(n+1, 0);
    loop(i, 0, n){
        int a, b;
        cin >> a >> b;
        freq[a]++, freq[b]++;
        if (a==b || freq[a]>2 || freq[b]>2) self = true;
