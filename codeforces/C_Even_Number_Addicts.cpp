...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        int n, q, alice=0, flag=0, maxi=LLONG_MIN, mini=LLONG_MAX;
        int in;
        cin >> n;
        vi v(n);
        ...
        if (flag==0 || flag&1 && (n-flag)&1 || flag%2==0 && (n-flag)%2==0 && flag>n-flag || flag%2 && (n-flag)%2==0 && flag>n-flag)
            alice = 1;

        if (alice) print("Alice");
        else print("Bob");
    }
    return 0;
}
