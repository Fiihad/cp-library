...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        int n, x, y, out=0, sum=0, flag=0, maxi=LLONG_MIN, mini=LLONG_MAX;
        int in;
        cin >> n >> x >> y;
        string a,b;
        cin >> a >> b;
        ...
        vi v;
        loop (i, n) if (a[i]!=b[i]) v.pb(i);
        if (v.size() & 1) {print(-1); continue;}

        if (y<=x) {
            if (v.size()==2 && v[0]+1==v[1] && y*2<x) print(y*2);
            else if (v.size()==2 && v[0]+1==v[1]) print(x);
            else print(y*v.size()/2);
        }
        else {
            for (int i=1; i<v.size(); i++) if (v[i]==v[i-1]+1) sum+=x, v[i-1]=-1, v[i]=-1, i++;
            for (int i=0; i<v.size(); i++) {
                if (v[i]==-1) continue;
                int j;
                for (j=i+1; j<v.size() && v[j]==-1; j++);
                if ((v[j]-v[i])*x < y) sum+=(v[j]-v[i])*x;
                else sum+=y;
                i=j;
            }
            print(sum);
