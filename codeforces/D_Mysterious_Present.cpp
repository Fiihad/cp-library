...
void solve(int cases)
{
    int n, cardh, cardw, out=0, sum=0, flag=0, maxi=LLONG_MIN, mini=LLONG_MAX;
    int in;
    cin >> n >> cardh >> cardw;
    ...
    loop(i,0,n){
        int h, w;
        cin >> h >> w;
        if (h > cardh && w > cardw) v.pb({h,w,i+1});
    }
    if (v.size() == 0){
        print(0);
        return;
    ...
    vector<pii> check; // size, index

    loop(i,0,v.size()){
        pii toStore={0,-1};
        loop(j,0,i){
            if (get<1>(v[j]) < get<1>(v[i]) && get<0>(v[j]) < get<0>(v[i])){
                if (check[j].first > toStore.first){
                    toStore.first = check[j].first;
                    toStore.second = j;
                }
            }
        }
        check.pb({toStore.first+1, toStore.second});
    }

    vi output;
    int maxsize = -1, maxind;
    loop(i,0,check.size()) if (check[i].first > maxsize) maxsize = check[i].first, maxind = i;
    while (1){
        output.pb(get<2>(v[maxind]));
        maxind = check[maxind].second;
        if (maxind==-1) break;
    }
    reverse(all(output));
