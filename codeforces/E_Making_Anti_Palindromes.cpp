...
void solve(int cases)
{
    ...
    map<pii, int> nopals;
    loop(i,0,n/2){
        if (in[i]==in[n-1-i]) pals[in[i]]++;
        else nopals[{in[i], in[n-1-i]}]++;
    }
...
#define printxp(array) { for (auto x : array) cout << '(' << x.first << ',' << gap << x.second << ')' << gap; cout << endl; }
    ...
    for (auto x: pals) sum += x.second;
    for (auto x: pals){
        if (x.second > sum-x.second){
            q = x.first;
            flag = x.second-(sum-x.second);
            sum -= flag;
            break;
        }
    }
    ...
    sum = 0;
    for (auto x: nopals)
        if (x.first.first != q && x.first.second != q) sum += x.second;
    if (flag <= sum){
        out += flag;
        print(out);
    }
