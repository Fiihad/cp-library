...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        loop(i,10){
            loop(j,koyta[i].size()) cout<<i;
            loop(j,jekono[i]) cout<<i;

            if (!koyta[i].empty()){
                for (int j=flag; j<koyta[i].back(); j++){
                    if (str[j]-'0' > i) jekono[min(str[j]-'0'+1, 9)]++, koyta[min(str[j]-'0', 9)].pop_front();
                }
                flag = koyta[i].back();
            }
        }
