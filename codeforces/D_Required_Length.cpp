...
int counting(int x, int n, int count){;
    if (m.count(x)) return m[x]+count;
    if (to_string(x).size()==n) { m[x] = 0; return count; }

    int mini = LLONG_MAX;
    for (int temp = x; temp; temp /= 10){
        int prev = mini;
        if (temp%10 == 1 || temp%10 == 0) continue;
        int check = x*(temp%10);
        mini = min(mini, counting(check, n, count+1));
    }
    if (mini==LLONG_MAX) return INT32_MAX;
    m[x] = mini-count;
    return mini;
}
