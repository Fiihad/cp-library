...
void dfs(int node, const vi &graph, vi &visited, vi &dp, vi &path) {
    visited[node] = 1;       // Mark node as visited
    path.push_back(node);    // Add node to the current path
    
    int next = graph[node];  // The next node in the graph
    if (!visited[next]) {
        dfs(next, graph, visited, dp, path);
    } else if (visited[next] == 1) {
        // If 'next' is part of the current path, we've found a cycle
        int start = path.size();
...
void solve(int cases)
{
    ...
    loop(i, 1, n + 1) {
        if (!visited[i]) {
            ...
            dfs(i, graph, visited, dp, path);
        }
    }

    auto rec = [&](int node, auto &&rec)->int {
        if (dp[node]!= -1) return dp[node];
        return dp[node] = rec(graph[node], rec) + 1;
    };

    loop(i, 1, n + 1) { maxi = max(maxi, rec(i, rec)); }
    print(maxi + 2);
}
