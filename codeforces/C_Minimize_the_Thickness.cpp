...
void solve(int cases)
{
    ...
    loop(i,0,n){
        if (v[n-1]%v[i]==0){
            maxi = i+1;
            flag = i;
            loop(j, 2, v.back()/v[i]+1){
                if (binary_search(v.begin()+i, v.end(), v[i]*j)){
                    int index = lower_bound(v.begin()+i, v.end(), v[i]*j) - v.begin();
                    int thickness = index - flag;
                    flag = index;
                    maxi = max(maxi, thickness);
                }
                else goto THERE;
            }
            mini = min(maxi, prev_maxi);
            prev_maxi = mini;
        }
        THERE:;
    }
