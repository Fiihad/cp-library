...
void solve(int cases)
{
    int n, q, out=0, sum=0, maxi=LLONG_MIN, mini=LLONG_MAX;
    int in;
    cin >> n;
    vi v(n), distincts(n+1,0);
    set<int> s;
    int flag = mini;

    loop(i,0,n){
        cin >> v[i];
        if (distincts[v[i]]==0) distincts[v[i]]=i+1;
        if (s.count(v[i]) && distincts[v[i]]<flag) flag = distincts[v[i]];
        s.insert(v[i]);
    }
    if (flag==mini) flag=1;
