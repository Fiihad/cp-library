...
void solve(int cases)
{
    ...
    while (q--){
        int l, r;
        cin >> l >> r;
        auto it1 = lower_bound(all(ones), l);
        auto it2 = lower_bound(all(zeros), r, greater<>());
        if (it1 == ones.end() || it2 == zeros.end() || *it1 >= *it2) pairs.insert({0, 0});
        else pairs.insert({*it1, *it2});
        // bug(*it1, *it2);
    }
    print(pairs.size());
}
