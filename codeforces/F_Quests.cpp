...
#define printx(array) { for (auto x : array) cout << x << '+'; cout << endl; }
...
// SOLVE STARTS HERE
int possible(vi &v, int n, int need, int day, int check)
{
    int sum = accumulate(v.begin(), v.begin()+min(n,check+1), 0LL);
    int work = (day/(check+1))*sum;
    work += accumulate(v.begin(), v.begin()+min(n, day%(check+1)), 0LL);
    bug(sum, work, check, need);

    if (work>=need) return 1;
    return 0;
}
...
void solve(int cases)
{
    ...
    if (accumulate(v.begin(), v.begin()+min(day,n), 0LL) >= need){
        print("Infinity");
        return;
    }
    ...
    while (hi>=lo){
        int mid = (hi+lo)/2;
        bug(mid);
        if (possible(v, n, need, day, mid)) out = mid, lo = mid+1;
        else hi = mid-1;
        bug(hi, lo, mid);
    }
