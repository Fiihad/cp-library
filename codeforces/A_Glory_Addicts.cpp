...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        int in;
        cin >> n;
        vi type(n), fire, frost;
        scanx(type);

        loop(i,0,n){
            cin >> in;
            if (type[i]) fire.pb(in);
            else frost.pb(in);
        }
        if (!fire.empty()) sort(all(fire));
        if (!frost.empty()) sort(all(frost));

        int fire_size=fire.size(), frost_size=frost.size();

        if (fire.size() > frost.size() or fire.size()==frost.size() and fire[0]<=frost[0]){
            out+=fire[0];
            fire[0]=0;
        }
        else if (fire.size() > frost.size() or fire.size()==frost.size() and fire[0]>frost[0]){
            out+=frost[0];
            frost[0]=0;
        }

        if (fire.size()>0) for(int i=fire.size()-1; i>=0; i--){
            if(frost_size-- >0) out+= 2*fire[i];
            else out+=fire[i];
        }
        if (frost.size()>0) for(int i=frost.size()-1; i>=0; i--){
            if(fire_size-- >0) out+= 2*frost[i];
            else out+=frost[i];
        }
