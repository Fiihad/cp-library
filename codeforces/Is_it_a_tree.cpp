...
public:
    ...
    bool dfs()
    {
        ...
        vector<bool> visited(n+1, false);
        int check = dfs_helper(1, 1, visited);
    ...
    bool dfs_helper(int source, int parent, vector<bool> &visited)
    {
        visited[source] = true;

        for (auto nbr: adj_list[source]){
            if (visited[nbr] && nbr!=parent) return false;
            if (!visited[nbr]) dfs_helper(nbr, source, visited);
        }
        return true;
    }
};
