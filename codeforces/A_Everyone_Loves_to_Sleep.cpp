...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        int n, q, in, out=0, sum=0, flag=0, h, m, sleep;
        cin >> n >> h >> m;
        sleep = 60*h+m;
        vi times(n);
        loop(i,n){
        ...
        sort(all(times));
        out = lower_bound(all(times), sleep) - times.begin();
        if (out==n) out = 24*60 - sleep + times[0];
        else out = times[out] - sleep;
        print(out/60 << gap << out%60);
    }
    return 0;
}
