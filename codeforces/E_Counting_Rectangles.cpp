...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        int n, q, in1, in2, out=0, sum=0, flag=0, maxi=LLONG_MIN, mini=LLONG_MAX;
        cin >> n >> q;

        memset(ara, 0, sizeof(ara));

        loop(i,n) cin>>in1>>in2, ara[in1][in2]+=in1*in2;
        ...
        while(q--){
            ...
            // bug(ara[c-1][d-1], ara[a][d-1], ara[c-1][b], ara[a][b]);
            print(ara[c-1][d-1] - ara[a][d-1] - ara[c-1][b] + ara[a][b]);
        }
    }
    return 0;
}
