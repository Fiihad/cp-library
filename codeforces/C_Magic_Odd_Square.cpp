...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        int n, q, out=0, sum=0, flage=0, flago=0;
        int in;
        cin >> n;

        vi odd, even;
        loop(i,1,n*n) if (i%2) odd.pb(i); else even.pb(i);

        int middle=n/2+1, oddcount=1;

        loop(i,1,n){
            loop(j,1,n){
                if (j>=middle-oddcount/2 && j<=middle+oddcount/2)
                    cout << odd[flago++] << gap;
                else cout << even[flage++] << gap;
            }

            cout << endl;
            if (i>=middle) oddcount-=2;
            else oddcount += 2;
        }
    }
    return 0;
