...
{
    int n, out=0, sum=0, flag=-1, maxi=LLONG_MIN, mini=LLONG_MAX;
    int in;
    char q;
    cin >> n >> q;
    string s;
    ...
    loop(i, 0, n){
        ...
        else if (s[i]==q) qs.pb(i);
    }

    loop(i, 0, qs.size()){
        int index = lower_bound(all(gs), qs[i]) - gs.begin();
        int distance;
        if (index==gs.size()) distance = n-qs[i]+flag;
        else distance = gs[index]-qs[i];
