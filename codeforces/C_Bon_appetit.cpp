...
void solve(int cases)
{
    ...
    priority_queue<int> customer_count;
    for (auto x: m) customer_count.push(x.second);
    ...
    while (!table.empty() && !customer_count.empty()){
        int customer = customer_count.top(), capacity = table.top();
        customer_count.pop();
        table.pop();
        if (customer < capacity){
            out += customer;
            // table.push(capacity-customer);
        }
        else if (customer == capacity){
            out += customer;
        }
        else {
            out += capacity;
            customer_count.push(customer-capacity);
        }
        // bug(customer, capacity);
    }
    print(out);
END:;
}
