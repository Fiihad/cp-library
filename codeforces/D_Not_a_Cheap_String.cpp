...
int32_t main()
{
    ...
    for (int cases=1; cases<=test_cases; cases++)
    {
        ...
        loop(n){
            sum += in[i]-96;
            temp.pb(make_pair(in[i], i));
        }
        sort(all(temp));

        // loop(n) cout << temp[i].first << gap << temp[i].second << endl;

        int p, i;
        cin >> p;
        for (i=n-1; sum>p; i--) sum -= (temp[i].first-96);
        for (int j=0; j<=i; j++) out[temp[j].second] = temp[j].first;

        loop(n) if (out[i]) cout << char(out[i]);
        cout << endl;
    }
    return 0;
}