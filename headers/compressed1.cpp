...
int n=a.size(); assert(n>=2);
vector<pair<pair<int,int>,int>>p(n);
for (int i=0; i<n; i++) p[i]={a[i],i};
sort(p.begin(),p.end()); int l=0,r=2;
...
int n; cin>>n; vector<pair<int,int>>p(n);
...
struct PT { double x,y; PT() { x=0,y=0; }
...
assert(orientation(a,b,c) !=0);if (orientation(a,c,b)<0) swap(b,c);return orientation(a,c,p)>=0 && orientation(a,b,p)<=0;}
...
sort(v.begin(),v.end(),[&](PT a,PT b) {return make_tuple(half(a-o),0.0,(a-o).norm2())<make_tuple(half(b-o),cross(a-o,b-o),(b-o).norm2());});} struct line {
PT a,b;//goes through points a and b
PT v; double c;/*line form: direction vec [cross] (x,y)=c*/line() {}/*direction vector v and offset c*/line(PT v,double c) : v(v),c(c) {auto p=get_points();a=p.first; b=p.second;}
/*equation ax+by+c=0*/line(double _a,double _b,double _c) : v({_b,-_a}),c(-_c) {auto p=get_points();a=p.first; b=p.second;}
/*goes through points p and q*/line(PT p,PT q) : v(q-p),c(cross(v,p)),a(p),b(q) {}
pair<PT,PT>get_points() {/*extract any two points from this line*/PT p,q; double a=-v.y,b=v.x;//ax+by=c
if (sign(a)==0) { p=PT(0,c/b);q=PT(1,c/b);} else if (sign(b)==0) { p=PT(c/a,0);q=PT(c/a,1);} else { p=PT(0,c/b); q=PT(1,(c-a)/b);} return {p,q};}
//ax+by+c=0 array<double,3>get_abc() {
...
if (c<0) return 1; if (c>0) return 2;
return 3;}
...
c1=circle(p1,r1); if (p1==p2) { c2=c1;
return 1;} c2=circle(p2,r1); return 2;}
...
double d=dist(p,q); double l=r*r/d;
...
return 0;} vector<pair<PT,PT>>out;
for (int tmp: {-1,1}) {
...
seg.clear(),cover.clear(); arc=pol=0;
} void init() { n=0;
seg.clear(),cover.clear(); arc=pol=0;}
...
seg.clear(); for (int j=0; j<n; j++) { if (i !=j) {
...
int n=p.size(); PT c(0,0); double sum=0;
...
auto tot_dist=[&](PT z) { double res=0;
for (int i=0; i<p.size(); i++) res+=dist(p[i],z); return res;
}; auto findY=[&](double x) {
...
if (a<0 || b>0) return 1; int l=1,r=n-1;
...
double ans=0; int i=0,j=1; while (i<n) {
...
double ans=inf; int i=0,j=1; while (i<n) {
...
vector<PT>ans; int n=(int)p.size();
for (int i=0; i<n; i++) {
...
line l=line(a,b); double ans=0.0;
...
double tmp=dot(inter,l.v); int f;
...
} else {double sa=cross(d-c,a-c),sb=cross(d-c,b-c);
...
int n=a.size(),m=b.size(); int i=0,j=0;
a.push_back(a[0]); a.push_back(a[1]);
b.push_back(b[0]); b.push_back(b[1]);
vector<PT>c; while (i<n || j<m) {
c.push_back(a[i]+b[j]);
double p=cross(a[i+1]-a[i],b[j+1]-b[j]);
if (sign(p)>=0)++i; if (sign(p)<=0)++j;
} return c;}
...
events.push_back({ed,-1}); if (st>ed) {
events.push_back({-PI,+1});
events.push_back({+PI,-1});}
} sort(events.begin(),events.end());
int cnt=0; for (auto &&e: events) {
cnt+=e.second; if (cnt>ans) { ans=cnt;
id=i; th=e.first;}}}
PT w=PT(p[id].x+r*cos(th),p[id].y+r*sin(th));
c=circle(w,r);//best_circle return ans;}
...
double l=0,r=20000; while (r-l>eps) {
double mid=(l+r)*0.5; vector<HP>h;
const int L=1e9;
...
double l=v[m]/2,r=1e6;/*fix it correctly*/int it=60;
auto ang=[](double x,double r) {//x=length of the chord,r=radius of the circle
return 2*asin((x/2)/r);
}; auto calc=[=](double r) {
double sum=0; for (auto x: v) {
sum+=ang(x,r);} return sum;};
//compute the radius of the circle
while (it--) { double mid=(l+r)/2;
if (calc(mid)<=2*PI) { r=mid;
} else { l=mid;}}
if (calc(r)<=2*PI-eps) {//the center of the circle is outside the polygon
auto calc2=[&](double r) { double sum=0;
for (int i=0; i<v.size(); i++) {
double x=v[i]; double th=ang(x,r);
if (i !=m) { sum+=th;
} else { sum+=2*PI-th;}
} return sum;
}; l=v[m]/2; r=1e6; it=60; while (it--) {
double mid=(l+r)/2;
...
p3 unit() { return*this/abs(); }}zero(0,0,0);
...
//p must be on the plane PT pos2d(p3 p) {
...
line3d(plane p1,plane p2) { d=p1.n*p2.n;
...
p3 n1=(b1-a1)*(c1-a1);
p3 n2=(b2-a2)*(c2-a2);
double d1=n1 | a1,d2=n2 | a2; p3 d=n1*n2;
...
plane P(a,b,c); p3 proj=P.proj(d);
...
double l=0.0,r=1.0; int cnt=100;
double ret=inf; while (cnt--) {
...
reverse(fs[u].begin(),fs[u].end());} return fs;}

//O(n^2),O(n) faces in the hull
struct CH3D { struct face {
...
vector<p3>P;int num;//convex hull surface triangle number
vector<face>F;//convex surface triangles
vector<vector<int>>g;
void init(vector<p3>p) { P=p; n=p.size();
F.resize(8*n+1);
...
p3 m=P[f.b]-P[f.a]; p3 n=P[f.c]-P[f.a];
p3 t=p-P[f.a]; return (m*n) | t;
...
add.ok=true; g[p][b]=g[a][p]=g[b][a]=num;
F[num++]=add;}}}
//recursively search all faces that should be removed from the convex hull
void dfs(int p,int now) { F[now].ok=0;
deal(p,F[now].b,F[now].a);
deal(p,F[now].c,F[now].b);
deal(p,F[now].a,F[now].c);
} bool same(int s,int t) {
p3 &a=P[F[s].a]; p3 &b=P[F[s].b];
p3 &c=P[F[s].c];
return fabs(volume(a,b,c,P[F[t].a]))<eps &&
fabs(volume(a,b,c,P[F[t].b]))<eps &&
fabs(volume(a,b,c,P[F[t].c]))<eps;}
//building a 3D convex hull
void create_hull() { int i,j,tmp;
face add; num=0; if (n<4)return;
...
add.b=(i+2) % 4; add.c=(i+3) % 4;add.ok=true;
...
dfs(i,j); break;}}
} tmp=num; for (i=num=0; i<tmp; i++)
if (F[i].ok) F[num++]=F[i];
} double surface_area() { double res=0;
if (n==3) { p3 p=cross(P[0],P[1],P[2]);
res=len(p)/2.0; return res;
} for(int i=0; i<num; i++) {
res+=area(P[F[i].a],P[F[i].b],P[F[i].c]);
} return res/2.0;
} double volume() { double res=0;
p3 tmp(0,0,0); for(int i=0; i<num; i++) {
...
for (j=0; j<i; j++) { if (same(i,j)) {flag=0; break;}
} res+=flag;} return res;
...
all+=vol;} ans=ans/all; return ans;}
...
/*s=r*theta return r*get_angle(a-o,b-o);*/}
...
int oa=sign(cd | a), ob=sign(cd | b),
oc=sign(ab | c), od=sign(ab | d);
...
assert(validSegment(a,b) && validSegment(c,d));p3 out;
if (proper_intersection(a,b,c,d,out)) return {out};DirectionSet s;
...
int n=p.size(); double sum=-(n-2)*PI;
for (int i=0; i<n; i++) {
...
double height(double th){return r*(1-cos(th));}
...
double h=height(th); return 2*PI*r*h;}};
...
double r=min(s1.r,s2.r); double y=R+r-d;
...
int n=p.size(); p3 c(0,0,0);
for(int i=0; i<n; i++) c=c+p[i]; c=c/n;

double ratio=0.1; int pos=0;
int it=100000; while (it--) { pos=0;
for (int i=1; i<n; i++) {
if(sq(c-p[i])>sq(c-p[pos])) pos=i;
} c=c+(p[pos]-c)*ratio; ratio*=0.998;
...
double th=acos(s.r/d); return th;}

struct pyramid {
int n;//number of sides of the pyramid
double l;//length of each side
double ang; pyramid(int _n,double _l) {
n=_n; l=_l; ang=PI/n;
} double base_area() {
...
struct cone {
double r,h;//radius and height
cone(double _r,double _h) { r=_r; h=_h;
} double volume() { return PI*r*r*h/3.0;
} double surface_area() {
...
double p=(a+b+c)/2;
return p*(p-a)*(p-b)*(p-c);} bitset<N>f;
//values are distinct
//find the maximum area triangle,given are lengths
int32_t main() {
int n,m; cin>>n>>m; f.reset();
for (int i=0; i<n; i++) { int k; cin>>k;f[k]=1;}
while (!f[m])--m; double ans=1e100;int a=1;
...
if (b<=m) { ans=min(ans,area(a,b,b+d));}
} if (ans<1e100) {
